...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; -webkit-user-select: none; user-select: none; }
  ...
  .orb {
    ...
    background: radial-gradient(circle at 35% 30%, #4c9cff, #2f6fe0 55%, #1d3f8c);
    box-shadow: 0 0 0 0 rgba(76,156,255,.45), 0 10px 40px rgba(20,60,140,.5);
    transition: transform .08s ease-out, box-shadow .3s ease; display: grid; place-items: center;
    text-align: center;
  }
  .orb .lbl { font-size: 15px; font-weight: 600; padding: 0 14px; pointer-events: none; }
  .orb.idle { background: radial-gradient(circle at 35% 30%, #46527a, #2a3650 55%, #1d2740); }
  ...
  button#end {
    width: 100%; padding: 14px; font-size: 15px; font-weight: 600; border: none; border-radius: 13px;
...
<body>
  ...
  <div class="orbwrap">
    <div class="orb idle" id="orb"><span class="lbl" id="orbLbl">Tap to start</span></div>
...
let pc=null, micStream=null, audioCtx=null, live=false, botBubble=null, rafId=null;
...
function addUser(text){ if(text&&text.trim()) addBubble("you","You · 你",text.trim()); }
function botDelta(delta){
  if(!botBubble){ botBubble=addBubble("tutor","Tutor · 私教",""); }
  botBubble.textContent+=delta; tr.scrollTop=tr.scrollHeight;
}
function botDone(finalText){
  if(botBubble&&finalText&&!botBubble.textContent) botBubble.textContent=finalText;
  botBubble=null;
}
...
async function start(){
  ...
  }catch(e){ setState("idle","Tap to start"); setHint("Tap, allow the mic, then just talk."); setErr("Mic blocked: "+(e.name||e)); return; }
  ...
  try{
    ...
    live=true; endBtn.style.display="block"; setState("listening","Listening"); setHint("Just talk. Tap the orb or End to stop.");
  }catch(e){ stop("error"); setErr("Voice connect failed: "+e.message); }
}

function onEvent(ev){
  const t=ev.type||"";
  if(t==="conversation.item.input_audio_transcription.completed"){ addUser(ev.transcript||""); }
  else if(t.indexOf("audio_transcript")>=0 && t.indexOf("delta")>=0){ setState("speaking","Speaking"); botDelta(ev.delta||""); }
  else if(t.indexOf("audio_transcript")>=0 && t.indexOf("done")>=0){ botDone(ev.transcript||""); }
...
function stop(reason){
  ...
  orb.style.transform="scale(1)"; setState("idle","Tap to start"); endBtn.style.display="none";
  if(reason==="user"){ setHint("Session ended."); addBubble("tutor","Idea Companion","Nice walk. In the full version I'll save this to your Notion and send a deeper dive. 下次见!"); }
